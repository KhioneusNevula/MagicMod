package com.gm910.magicmod.world.chunkgen;
...
public class ChunkGeneratorTyphonic2 implements IChunkGenerator {
	protected static final IBlockState AIR = Blocks.WATER.getDefaultState();
    protected static final IBlockState NETHERRACK = Blocks.PRISMARINE.getDefaultState();
    protected static final IBlockState BEDROCK = Blocks.BARRIER.getDefaultState();
    protected static final IBlockState LAVA = Blocks.WATER.getDefaultState();
    protected static final IBlockState GRAVEL = Blocks.SEA_LANTERN.getDefaultState();
    protected static final IBlockState SOUL_SAND = Blocks.SEA_LANTERN.getDefaultState();
    ...
    private final WorldGenerator quartzGen = new WorldGenMinable(Blocks.CLAY.getDefaultState(), 14, BlockMatcher.forBlock(Blocks.NETHERRACK));
    private final WorldGenerator magmaGen = new WorldGenMinable(Blocks.GRASS.getDefaultState(), 33, BlockMatcher.forBlock(Blocks.NETHERRACK));
    private final WorldGenHellLava lavaTrapGen = new WorldGenHellLava(Blocks.FLOWING_WATER, true);
    ...
    public ChunkGeneratorTyphonic2(World worldIn, boolean p_i45637_2_, long seed)
    {
        this.world = worldIn;
            ...
            for (int k = 0; k < 16; ++k)
            {
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    if (j1 < 127 - this.rand.nextInt(5) && j1 > this.rand.nextInt(5))
                    {
                        ...
                        if (iblockstate2.getBlock() != null && iblockstate2.getMaterial() != Material.WATER)
                        {
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        for (int x1 = 0; x1 < 16; ++x1) {
        	for (int z1 = 0; z1 < 16; z1++) {
	        	for (int y = 0; y < 255; ++y) {
	        		if (chunkprimer.getBlockState(x1, y, z1).equals(Blocks.AIR.getDefaultState()))
	        		chunkprimer.setBlockState(x1, y, z1, AIR);
	        		//Blocks.FLOWING_WATER.updateTick(world, new BlockPos(x1, y, z1), chunkprimer.getBlockState(x1, y, z1), rand);
	        	}
        	}
        }
